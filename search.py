...
class BeamSearch(object):
    ...
    def __init__(self, samples):
        ...
        self.topical_context_names=['topical_attended','topical_attended_mask'];
        #self.topical_context_names= self.generator.generate.topical_word_contexts
        self.state_names = self.generator.generate.states
        self.topical_names=['topical_embeddingq'];
        self.content_names=['content_embedding'];
        ...
        self.topical_contexts = [
            VariableFilter(bricks=[self.generator],
                           name=name,
                           roles=[INPUT])(self.inner_cg)[0]
            for name in self.topical_context_names]

        self.topical_embeddings=[
            VariableFilter(bricks=[self.generator],
                           name=name,
                           roles=[INPUT])(self.inner_cg)[0]
            for name in self.topical_names];
        self.content_embeddings=[
            VariableFilter(bricks=[self.generator],
                           name=name,
                           roles=[INPUT])(self.inner_cg)[0]
            for name in self.content_names];
        self.input_states = []
    ...
    def _compile_next_state_computer(self):
        ...
        self.next_state_computer = function(
            self.contexts +self.topical_embeddings+self.topical_contexts+self.content_embeddings+ self.input_states + next_outputs, next_states, on_unused_input='warn')
    ...
    def _compile_logprobs_computer(self):
        ...
        self.logprobs_computer = function(
            self.contexts +self.topical_embeddings+self.topical_contexts+self.content_embeddings+self.input_states, logprobs,
    ...
    def compute_initial_states_and_contexts(self, inputs):
        ...
        topical_embeddings = OrderedDict((n, outputs.pop(n)) for n in self.topical_names);
        topical_contexts = OrderedDict((n, outputs.pop(n)) for n in self.topical_context_names)
        content_embeddings = OrderedDict((n, outputs.pop(n)) for n in self.content_names);
        beam_size = outputs.pop('beam_size')
        initial_states = outputs
        return contexts, topical_embeddings,topical_contexts,content_embeddings,initial_states, beam_size

    def compute_logprobs(self, contexts, topical_embeddings,topical_contexts,content_embeddings, states):
        ...
        return self.logprobs_computer(*(list(contexts.values()) +topical_embeddings.values()+topical_contexts.values()+content_embeddings.values()+
                                      input_states))
    ...
    def search(self, input_values, eol_symbol, max_length,
        ...
        contexts, topical_embeddings,topical_contexts,content_embeddings,states, beam_size = self.compute_initial_states_and_contexts(
            input_values)
        ...
        for i in range(max_length):
            ...
            logprobs = self.compute_logprobs(contexts, topical_embeddings,topical_contexts,content_embeddings,states)
            ...
            states.update(self.compute_next_states(contexts, topical_embeddings,topical_contexts,content_embeddings,states, outputs))
